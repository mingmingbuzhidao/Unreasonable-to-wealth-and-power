...
{  
    ...
    {  
        ...
        {  
            ...
            //分为四种情况:  
            //1)既是叶子结点又是     根节点且 node.sizeOfKeys() < Constants.MIN_KEY_SIZE  
            //2)叶子结点但          非根结点且node.sizeOfKeys() < Constants.MIN_KEY_SIZE ----->只有这一种需要合并  
            //3)既是叶子结点又是     根节点且node.sizeOfKeys() > Constants.MIN_KEY_SIZE  
            //4)叶子结点但          非根结点且node.sizeOfKeys() > Constants.MIN_KEY_SIZE  
     ...
     * 需要分情况讨论： <br> 
     * 1) 此结点的右邻居存在且右邻居结点中关键字个数 >= minSize + 1  &nbsp;&nbsp;  相当于左旋  &clubs; <br> 
     * 2) 此结点的左邻居存在且左邻居结点中关键字个数 >= minSize + 1  &emsp;相当于右旋      &hearts;<br> 
     * 3) 此结点的右邻居存在且父节点关键字个数大于0 &emsp;结点合并  &spades;<br> 
     * 4) 此结点的左邻居存在且父节点关键字个数大于0 &emsp;结点合并  &diams;<br> 
     * @param node BTNode 
     */  
        ...
        {  
            ...
            {  
                ...
                {  
                    ...
                    {  
                        return node;  
                    }else if(key.compareTo(node.getKey(i )) > 0 && key.compareTo(node.getKey(i+1)) < 0)  
                    {  
...
//      right.setKeys(copy2);   //----2  
    ...
    {  
        ...
        {  
            bTree.insertKey(data[i]);  
        }  
        for(int i = 14 ; i >=0; i--)  
        {  
            bTree.removeKey(data[i]);  
        }  
